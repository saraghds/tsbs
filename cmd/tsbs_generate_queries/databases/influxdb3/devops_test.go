...
func TestHighCPUForHosts(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			desc:               "zero hosts",
			nHosts:             0,
			expectedHumanLabel: "InfluxDB3 CPU over threshold, all hosts",
			expectedHumanDesc:  "InfluxDB3 CPU over threshold, all hosts: 1970-01-01T00:16:22Z",
			expectedHypertable: "cpu",
		...
		},
		{
			desc:               "one host",
			nHosts:             1,
			expectedHumanLabel: "InfluxDB3 CPU over threshold, 1 host(s)",
			expectedHumanDesc:  "InfluxDB3 CPU over threshold, 1 host(s): 1970-01-01T00:47:30Z",
			expectedHypertable: "cpu",
		...
		},
		{
			desc:               "five hosts",
			nHosts:             5,
			expectedHumanLabel: "InfluxDB3 CPU over threshold, 5 host(s)",
			expectedHumanDesc:  "InfluxDB3 CPU over threshold, 5 host(s): 1970-01-01T00:08:59Z",
			expectedHypertable: "cpu",
...
func verifyQuery(t *testing.T, q query.Query, humanLabel, humanDesc, sqlQuery string) {
	tsq, ok := q.(*query.InfluxDB3)

	if !ok {
		t.Fatal("Filled query is not *query.InfluxDB3 type")
	}
